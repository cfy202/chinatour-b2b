            ...
            <div class="row">
    			<div class="col-md-12">
					<div class="block-flat">
						...
						<div class="content">
							<form method="post" id="formId" action="[@spring.url '/admin/payCostRecords/accSettlementAll.jhtml?menuId=505'/]">
...
<script type="text/javascript">
     $(document).ready(function () {
      ...
      $('input').iCheck({
        ...
        var oTable = $('#datatable2').dataTable({
            ...
            "ajax": {
                url: "[@spring.url '/admin/payCostRecords/findOrderTotalTaxList.jhtml'/]",
                type: "POST",
                "data": function ( data ) {
					...
					data.bookingBeginningDate = $("#search_beginningDate").val();
	                data.bookingEndingDate = $("#search_endingDate").val();
           ...
           "columns": [
                ...
                { "data": "orderNo", 
					"render" : function(data, type, row) {
						var html="";
                		if(row.tax==3){
                 			html="<div class='radio'><input type='checkbox'  name='ordersTotalIds' value='"+row.ordersTotalId+"' class='icheck'/><span class='color-warning'>"+data+"</span></div>";
				...
				{ "data": "payCost",
					"render":function(data, type, row) {
						if(data!=null){
							return (row.commonTourFee-data.payTotalSum).toFixed(2)
						}else{
							return (row.commonTourFee-0).toFixed(2)
						}
					}
				},
                { "data": "payCost",
				...
				/*{ "data": "payCost",
					"render":function(data, type, row) {
						if(data!=null&&row.state!=5&&row.state!=6){
		 ...
		 /* "fnCreatedRow": function( nRow, aData, iDataIndex ) {
			...
			"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
				if(aData.ordersTotalId==null){
					...
					/*if(aData.priceExpression!=null){
						$('td:eq(10)', nRow).html((aData.priceExpression).toFixed(2));
					}*/
					$('td:eq(10)', nRow).html((aData.payTotalSum-aData.costTotalSum).toFixed(2));
					$('td:eq(11)', nRow).html("");
					$('td:eq(12)', nRow).html("");
	...
	function initAddHtml($addHtml){
		...
		function printOrder(){
			...
			window.open("[@spring.url '/admin/payCostRecords/findOrderTotalTaxListPrint.jhtml'/]?tourCode="+tourCode+"&orderNo="+orderNo+"&userName="+userName+"&tax="+tax+"&sprCheck="+sprCheck+"&bookingBeginningDate="+beginningDate+"&bookingEndingDate="+endingDate+"&settlementDateBeg="+settlementDateBeg+"&settlementDateEnd="+settlementDateEnd+"&arrivalBeginningDate="+arrivalBeginningDate+"&arrivalEndingDate="+arrivalEndingDate+"&isSelfOrganize="+isSelfOrganize+"&peerId="+peerId);
		}
		/*格式化金额*/
		function fmoney(s, n)  
		{  
