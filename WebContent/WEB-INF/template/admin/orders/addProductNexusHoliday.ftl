                ...
                <div class="col-md-12">
                    <div class="block-flat">
						<ul class="nav nav-tabs">
							<li><a onclick="javascript:window.location.href='groupLine.jhtml?brand=chinatour&ordersTotalId=${ordersTotalId}'" data-toggle="tab">chinatour</a></li>
							...
							<li class="active"><a data-toggle="tab">InterTrips</a></li>
				        ...
				        <div class="params-cont" style="text-align: right;"> 
				        	...
				        	<select id="operaterSelect" onchange="changeOperater(this);">
				        		[#list operaterList as operater]
				        			<option value="${operater}" type="${operater}">${operater}</option>
				        		[/#list]
				        	</select>
				        	<select id="sourceSelect" onchange="changeSource(this);">
                        ...
                        <div class="content">
                            <div class="table-responsive">
                                ...
                                <div id="indeTable" margin="20px,auto">
                                ...
                                <table class="table table-bordered" style="display:none" id="otherTable">
							        <thead>
							            ...
							            <tr>
							                <th>Flight</th>
							                <th><a href="bookProduct.jhtml?ordersTotalId=${ordersTotalId}&brand=InterTrips&singleType=flight">book</a></th>
							            </tr>
							            <tr>
							                <th>Hotel</th>
							                <th><a href="bookProduct.jhtml?ordersTotalId=${ordersTotalId}&brand=InterTrips&singleType=hotel">book</a></th>
							            </tr>
							            <tr>
							                <th>Cruise</th>
							                <th><a href="bookProduct.jhtml?ordersTotalId=${ordersTotalId}&brand=InterTrips&singleType=cruise">book</a></th>
							            </tr>
							        </thead>
							    </table>
                            </div>
				...
				<table style="padding:10px;border: 0px none" width="100%">
					<tbody>
						...
						<tr>
							...
							<td>
								<input class="form-control" name="brand" value="InterTrips" readOnly="readOnly" type="text">
							</td>
						</tr>
						<tr>
							<td>
...
<script type="text/javascript">
    $(document).ready(function () {
       ...
       $("#datatable2").attr("width","100%");
        ...
        var oTable = $('#datatable2').dataTable({
            ...
            "bFilter":false,
            "ajax": {
                url: "[@spring.url '/admin/orders/groupLineList.jhtml?brand=InterTrips&isSystem=1'/]",
                type: "POST",
                "data": function ( d ) {
            ...
            "columnDefs" : [{
				"render" : function(data, type, row) {
					return '<a href="bookProduct.jhtml?ordersTotalId=${ordersTotalId}&brand=InterTrips&groupLineId='+ data +'">book</a>';
                 },
				"targets" : 3
		   }]
		});
    ...
    function changeOperater(operater){
    	...
    	if($option.attr("value") == 'other'){
    		...
    		$("#indeTable").hide();
    		$("#sourceSelect").empty();
    		$.post('venderList.jhtml',{'brand':'InterTrips','operater':$option.attr("value")},function(map){
    			var html='';
    ...
    function saveIndependentProduct(){
		$.ajax({
			...
			success:function(groupLineId){
				...
				$tds.eq(2).html("InterTrips");
				$tds.eq(3).html($("#tripDescInput").val());
				$tds.eq(4).html('<a href="bookProduct.jhtml?ordersTotalId=${ordersTotalId}&brand=InterTrips&groupLineId='+ groupLineId +'">booking</a>');
