                    ...
                    <div class="block-flat">
                        <div class="header">
							...
							<div class="options" style="margin:10px; padding:5px 0;">
									<input type="hidden" id="peerUserIdSpan" value="2"/>
									<!--此处修改，便于agency打印-->
									<input type="hidden" id="isSelfOrganizeSpan" value=""/>
									<div  class="nav-panel">
										<div  class="nav-block">
										...
										<div  class="nav-block">
											...
											<div  class="block-body default-4-line">
												<div  class="params-cont"> 
													<a style="cursor:pointer;"  target="_self"  class=" param-item " > 
														<span  class="unchecked" name="cost" checked="false" onclick="change(this,0);" >UNPAID</span> 
													</a> 
													<a style="cursor:pointer;"  target="_self"  class=" param-item " > 
														<span  class="unchecked" name="cost" checked="false" onclick="change(this,1);" >PAYING</span> 
													</a> 
													<a style="cursor:pointer;"  target="_self"  class=" param-item " > 
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        {	
            ...
            sOut += '<tr><td style="border-left: 1px solid #DADADA;border-bottom: 1px solid #dadada;">Income:</td><td style="border-left: 1px solid #DADADA;border-bottom: 1px solid #dadada;">'+pay+'</td></tr>';
            sOut += '<tr><td style="border-left: 1px solid #DADADA;border-bottom: 1px solid #dadada;">Cost:</td><td style="border-left: 1px solid #DADADA;border-bottom: 1px solid #dadada;">'+cost+'</td></tr>';
        ...
        var oTable = $('#datatable2').dataTable({
            ...
            "columns": [
                ...
                { "data": "orderNo",
                	"render" : function(data, type, row) {
						if(row.reviewState==1){
							return '<i title="New" class="fa fa-clock-o"/>'+row.orderNo+'';
						}else if(row.reviewState==5){
							return '<span><i title="Update" class="fa fa-clock-o"/><span>'+row.orderNo+'';
						}else{
							return row.orderNo;
						}
					}
                ...
                { "data": "itInfo",
                	"render" : function(data, type, row) {
                	if(data==1){
							return '<span class="color-danger">Final Voucher</span>';
						}else{
							return '';
						}
					}
	...
	function cancelOrder(buttonId, orderId, tourId){
		$.post("asynchronousCancelOrder.jhtml",{"orderId" : orderId},function(result){
			if(result == 'success'){
				...
				var $stateTd = $buttonLi.parent().parent().parent().prev().prev().prev().prev();
				if(tourId == ''){
				//如果订单未组团
