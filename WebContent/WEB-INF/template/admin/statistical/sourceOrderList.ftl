                ...
                <div class="col-md-12">
                    <div class="block-flat">
                        <div class="header" style="font-family:'Arial';font-size:16px;font-weight:bold;">
							...
							<input type="hidden" id="sourceName" value="${order.sourceName}"/>
                        ...
                        <div class="th-box" style="margin-top:-5px;">
							...
							<div class="th-box-b">
								<table style="font-family:'Arial';width:95%;margin:0 auto 0;">
									<tbody>
										<tr>
											<td style="height:20px;font-size:15px;font-weight:bold;padding:0 0 0 8px;">Passenger：${orders.totalPeople}</td>
											<td style="height:20px;font-size:15px;font-weight:bold;padding:0 0 0 8px;">Amount：${orders.commonTourFee}</td>
											<td style="height:20px;font-size:15px;font-weight:bold;padding:0 0 0 8px;">Income：${orders.pay}</td>
											<td style="height:20px;font-size:15px;font-weight:bold;padding:0 0 0 8px;">Cost：${orders.cost}</td>
											<td style="height:20px;font-size:15px;font-weight:bold;padding:0 0 0 8px;">Profit：${((orders.pay)!0)-((orders.cost)!0)}</td>
											<td style="height:20px;font-size:15px;font-weight:bold;padding:0 0 0 8px;">Open Balance：${((orders.commonTourFee)!0)-((orders.pay)!0)}</td>
										</tr>
									</tbody>
								</table>
							</div>
...
<script type="text/javascript">
	...
	function formatAsText(item){
        ...
        var oTable = $('#datatable2').dataTable({
            ...
            "ajax": {
                url: "[@spring.url '/admin/statistical/sourceDetails.jhtml'/]",
                type: "POST",
                "data": function ( data ) {
	                data.time = $("#time").val();
	                data.year = $("#year").val();
	                ...
	                data.sourceName = $("#sourceName").val();
	                data.wr = $("#wr").val();
    ...
    function printOrder(){
        ...
        var role = $("#role").val();
        if(time.indexOf("-")<0){
        	location.href="${base}/admin/statistical/sourceDetailsPrint.jhtml?role="+role+"&orderType="+orderType+"&year="+year+"&deptId="+deptId+"&groupLineId="+groupLineId+"&isSelfOrganize="+isSelfOrganize+"&userId="+userId+"&peerId="+peerId+"&sourceName="+sourceName+"&wr="+wr+"&ticketType="+ticketType;
        }else{
        	location.href="${base}/admin/statistical/sourceDetailsPrint.jhtml?role="+role+"&orderType="+orderType+"&time="+time+"&deptId="+deptId+"&groupLineId="+groupLineId+"&isSelfOrganize="+isSelfOrganize+"&userId="+userId+"&peerId="+peerId+"&sourceName="+sourceName+"&wr="+wr+"&ticketType="+ticketType;
        }
    }
</script>
</body>
