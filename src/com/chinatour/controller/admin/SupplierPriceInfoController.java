 ...
 */
@Controller
@RequestMapping("/admin/supplierPriceInfo")
public class SupplierPriceInfoController extends BaseController {
	...
	private String SUPPLIER_TYPE = "1";// 供应商类型 1代表地接	
	/**
	 * 加载
	 */
	@RequestMapping(value = "/orderSupplierAdd", method = RequestMethod.GET)
	public String orderFlightAdd(ModelMap model, Tour tour) {
		...
		return "/admin/orderSupplier/orderSupplierAdd";
	}
	/**
	 * OP Group录账单功能
	 * @param model
	 ...
	 * @return
	 */
	@RequestMapping(value = "/orderSupplierAddGroup", method = RequestMethod.GET)
	public String orderFlightAddGroup(ModelMap model, Tour tour) {
		...
		model.addAttribute("customerList",
				supPriceInfoRelService.findCustomerByTourId(tour.getTourId()));
		return "/admin/orderSupplier/orderSupplierAddGroup";
	}
	/**
	 * 保存数据
	 */
	...
	public String save(ModelMap model, SupplierOfAgentVO supplierOfAgentVO,
		...
		for(int i=0;i<agentOfOrderList.size();i++){
			...
			for (int j = 0; j < orderOfOrderList.size(); j++) {
				if (agentOfOrderList.get(i).getUserId().equals(orderOfOrderList.get(j).getUserId())) {
					...
					//supplierOfAgentVO.getSupplierPriceInfoList().get(j).setOrderId(orderOfOrderList.get(j).getId());
					
					for (int k = 0; k < supplierOfAgentVO.getSupplierPriceInfoList().size(); k++) {
						if (agentOfOrderList.get(i).getUserId().equals(supplierOfAgentVO.getSupplierPriceInfoList().get(k).getUserId()) && orderOfOrderList.get(j).getId().equals(supplierOfAgentVO.getSupplierPriceInfoList().get(k).getOrderId())) {
							
							supplierOfAgentVO.getSupplierPriceInfoList().get(k).setSupplierPriceInfoId(UUIDGenerator.getUUID());
							supplierOfAgentVO.getSupplierPriceInfoList().get(k).setSupplierOfAgentId(supplierOfAgentS.getSupplierOfAgentId());
							BigDecimal totalsum=supplierOfAgentVO.getSupplierPriceInfoList().get(k).getTourPrice().add(supplierOfAgentVO.getSupplierPriceInfoList().get(k).getSinglePrice()).add( supplierOfAgentVO.getSupplierPriceInfoList().get(k).getChildrenPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getFlyPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getSpecialPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getDelayPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getPreferentialPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getBedPrice());
							if(!totalsum.equals(BigDecimal.ZERO)){
								sum =sum.add(totalsum);
							}
							if(!supplierOfAgentVO.getSupplierPriceInfoList().get(k).getIsDel().equals("1")){
								totalPeople++;
								customerNos+=(supplierOfAgentVO.getSupplierPriceInfoList().get(k).getCustomerNo()+",");
							}
		...
		supPriceInfoRelService.save(supplierPrice, supPriceInfoRel, supplierOfAgentList, supplierCheckList, supplierOfAgentVO.getSupplierPriceInfoList(), null, null, null, supplierOfOrderList);
	...
	public String updateSupplier(ModelMap model, Tour tour,SupPriceInfoRel supPriceInfoRel) {
		model.addAttribute("menuId", "404");
		model.addAttribute("Tour", tour);
		model.addAttribute("supplierS", supplierService.findAll());
		SupplierPriceInfo supplierPriceInfo =new SupplierPriceInfo();
		//supplierPriceInfo.setUserId(adminService.getCurrent().getId());
	 ...
	 * 跳转修改酒店账单
	 */
	@RequestMapping(value = "/updateSupplierGroup", method = RequestMethod.GET)
	public String updateSupplierGroup(ModelMap model, Tour tour,SupPriceInfoRel supPriceInfoRel) {
		model.addAttribute("menuId", "414");
		model.addAttribute("Tour", tour);
		model.addAttribute("supplierS", supplierService.findAll());
		SupplierPriceInfo supplierPriceInfo =new SupplierPriceInfo();
		...
		model.addAttribute("supPriceInfoRel" , supPriceInfoRelService.findById(supPriceInfoRel.getSupPriceInfoRelId()));
		model.addAttribute("supplierPrice" , supplierPriceService.findByTourId(tour.getTourId()));
		return "/admin/orderSupplier/orderSupplierUpdateGroup";
	}
	...
	public String update(ModelMap model,SupplierOfAgentVO supplierOfAgentVO,
		...
		for (int i = 0; i < supplierOfAgentList.size(); i++) {
			...
			for (int j = 0; j < orderOfOrderList.size(); j++) {
				...
				// 每个Agent收客人数
				for (int k = 0; k < supplierOfAgentVO.getSupplierPriceInfoList().size(); k++) {
					if (orderOfOrderList.get(j).getId().equals(supplierOfAgentVO.getSupplierPriceInfoList().get(k).getOrderId())) {
						BigDecimal totalsum = supplierOfAgentVO.getSupplierPriceInfoList().get(k).getTourPrice().add(supplierOfAgentVO.getSupplierPriceInfoList().get(k).getSinglePrice()).add( supplierOfAgentVO.getSupplierPriceInfoList().get(k).getChildrenPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getFlyPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getSpecialPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getDelayPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getPreferentialPrice() ).add(  supplierOfAgentVO.getSupplierPriceInfoList().get(k).getBedPrice());
		...
		supPriceInfoRelService.updateSupplier(supplierPrice, supPriceInfoRel,
				supplierOfAgentList, supplierCheckList, supplierOfAgentVO.getSupplierPriceInfoList(), null, null, null,
